#=
Reg1.jl
Regularizer, based on Reg1.m
Copyright 2006-12-6, Jeff Fessler, University of Michigan
2019-11-29 Julia 1.2
=#

"""
    R = Reg1(kappa, [options])
...
General form of (possibly nonquadratic) penalty function:
 `R(x) = \\sum_{m=1}^M \\sum_n w[n;m] ψ_m( [C_m x]_n )`
where `M` is the number of neighbors (offsets), and `n=1,...,N=length(x)`
and `C_m` is a `N × N` differencing matrix in the direction `offset_m`.

See `Rweights()` for `w[n;m]` options, only some of which use low memory.

Penalty gradient is `\\sum_m C_m' D_m(x) C_m x`,
where `D_m(x) = diag{w[n;m] \\wpot_[n;m]([C_m x]_n)}`
and `\\wpot(t) = ψ(t) / t`
...
* `R` struct with methods:
  ...
  - `R.cgrad(x)` evaluates `∇ R(x)` (column gradient)
  ...
  - `(pderiv,pcurv) = R.dercurv(R.C1*x)` derivatives and curvatures
  ...
  - `R.C` `Diagonal(sqrt(w)) * C1`, only for quadratic case!
  - `R.wt` see `Rweights()`

Typical use:
- `mask = trues(128, 128)` # or something more conformal
...
function Reg1(kappa::AbstractArray{<:Number} ;
	...
	#	edge_type = :simple, # saves memory
		...
		pot_arg::Any = (:quad),
		beta::Real = 2^0,
		type_denom::Symbol = :none,
		pre_denom_sqs1_x0::Bool = false,
		distance_power::Real = 1, # perhaps '2' would be better
		wt_use_mex::Bool = false,
		user_wt = [], # for Rweights
		order::Int = 1, # 1st-order differences
		mask::AbstractArray{<:Bool} = kappa .> 0,
		offsets::AbstractVector = [],
		offsets_is_zxy::Bool = false,
		control::Any = [], # todo: document this
		nthread::Int = 1, # todo: cut no mex
