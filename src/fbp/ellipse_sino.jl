...
"""
`(sino, pos, ang) = ellipse_sino(sg, ells ; oversample=1, xscale=1, yscale=1)`
...
out
- `sino`		`[nb na]` sinogram
- `pos` 		`[nb]` radial sample positions (or [nb na] if mojette)
...
#=
		if sg.how == :fan
			(sino, pos, ang) = ellipse_sino_go(ells, sg.nb, sg.ds, sg.offset,
					sg.na, sg.orbit, sg.orbit_start,
					sg.dso, sg.dod, sg.dfs, sg.source_offset,
					xscale, yscale, oversample, 0)
		elseif sg.how == :par
			(sino, pos, ang) = ellipse_sino_go(ells, sg.nb, sg.dr, sg.offset,
					sg.na, sg.orbit, sg.orbit_start,
					Inf, 1, 0, sg.source_offset,
					xscale, yscale, oversample, 0)
		elseif sg.how == :moj
			(sino, pos, ang) = ellipse_sino_go(ells, sg.nb, sg.d, sg.offset,
					sg.na, sg.orbit, sg.orbit_start,
					Inf, 1, 0, sg.source_offset,
					xscale, yscale, oversample, 0)
	else
...
=#
	...
	return ellipse_sino_go(ells, sg.nb, sg.d, sg.offset,
				sg.na, sg.orbit, sg.orbit_start,
				sg.dso, sg.dod, sg.dfs, sg.source_offset,
				xscale, yscale, oversample, sg.how == :moj) # todo
end
...
function ellipse_sino_go(ells, nb, ds, offset_s, na, orbit, orbit_start,
	dso, dod, dfs, source_offset, xscale, yscale, oversample, is_mojette)

	ang = (orbit_start .+ (0:(na - 1))' / na * orbit) * (pi/180)
	(pos, pos2) =
		ellipse_sino_pos(pos[:], nb, ds, offset_s, oversample, is_mojette, ang)
...
"""
function ellipse_sino_pos(pos, nb, ds, offset_s, nover, is_mojette, ang)
	wb = (nb - 1)/2 + offset_s
...
"""
function ellipse_sino_do(ells, pos, ang, xscale, yscale,
