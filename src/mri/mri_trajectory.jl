...
using MIRT: image_geom_mri, ndgrid, mri_kspace_spiral, ir_mri_kspace_ga_radial
=#
...
function mri_trajectory( ;
    ktype::Symbol = :radial,
    N::Dims = (32,30),
    fov::Union{Real,AbstractVector{<:Number}} = [250,250],
  ...
  if (length(fov) == 1)
    ...
    if (ktype == :radial)
      omega, wi = mri_trajectory_radial(N, fov ; kwargs...)
...
function mri_trajectory_radial(N::Dims, fov ;
    nr::Int = maximum(N)/2,
    ir::AbstractArray = 0:nr,
    na_nr::Real = 2π,
    na::Int = Int(4*ceil(na_nr * nr/4)), # mult of 4
  )
...
function mri_trajectory(test::Symbol)
  ...
  for ktype in ktypes
    ...
    kspace, omega, wi = mri_trajectory( ;
       ktype=ktype, N=N, fov = ig.fovs, arg_wi = arg_wi, args...)
    p = scatter(omega[:,1]/π, omega[:,2]/π, aspect_ratio=1,
			xlim = [-1,1]*1.1, xtick = -1:1,
			ylim = [-1,1]*1.1, ytick = -1:1,
            xlabel = "omega1/π", ylabel = "omega2/π", title = string(ktype), label="")
    push!(plots, p)
  end
  plot(plots...); gui()
